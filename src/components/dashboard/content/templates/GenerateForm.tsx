...
export function GenerateForm({ templates, onQueued }: GenerateFormProps) {
  ...
  const [prompt, setPrompt] = useState("Lorraine Hawkins scalp detox masterclass");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 text-sm text-black/70">
      <div>
        ...
        <p className="mb-1 text-[10px] text-black/50">Select a pre-made template structure.</p>
        <select
          value={templateId}
      ...
      <div>
        ...
        <p className="mb-1 text-[10px] text-black/50">Provide specific context or topics for this generation.</p>
        <textarea
          value={prompt}
